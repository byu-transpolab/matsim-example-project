...
fac <- read_csv("R/facilities/AddressPoints.csv") %>%
  filter(PtType == "Commercial" | PtType == "Other")
         #| PtType == "Agricultural") %>%
  colnames(fac)[1] <- "TAZ"
  
tazc <- read_csv("R/facilities/tazcentroids.csv") %>%
  arrange(TAZID)
  colnames(tazc)[1] <- "TAZ"
```
...
facility <- left_join(tazc, fac, by = "TAZ") %>%
  mutate(facility_id = 1:n(),
         ...
         lat = ifelse(is.na(xcoord.x), xcoord.y, xcoord.x),
         lon = ifelse(is.na(ycoord.x), ycoord.y, ycoord.x)) %>%
