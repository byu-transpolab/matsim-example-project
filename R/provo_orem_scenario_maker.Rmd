...
## nothing in this hhcoord file is in utah county... only salt lake and weber counties.
hh <- households %>%
  filter(latitude < 40.30416667 & longitude > -111.72138889,
         latitude > 40.21194444 & longitude < -111.61972222)

hh <- households %>%
  filter(latitude < 43 & latitude > 38,
         longitude < -110 & longitude > -114)
...
hh_visual <- st_as_sf(households, coords = c("longitude", "latitude"), crs = 4326)
mapview(hh_visual)
```
...
households %>% 
  filter(household_id %in% sample_persons$household_id) %>%
  write_csv("scenarios/provo_orem/households_sample.csv")
```
