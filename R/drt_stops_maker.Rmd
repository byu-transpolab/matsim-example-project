...
center_orem <- st_read("Provo_Orem_Stops.geojson")
...
Here, we further manipulate the data into the table format we want. We want to create two stops at each coordinate, so we duplicate, and further define the stops.

```{r}
is.odd <- function(x) x %% 2 != 0

center_orem <- center_orem %>% slice(rep(1:n(), each = 2)) %>%
  mutate(
    Index = 1:n(),
    link_idr = ifelse(is.odd(Index), link_id, paste(link_id,"r",sep="")),
    stopFacility = ifelse(is.odd(Index), paste(link_id,"_a",sep=""), paste(link_id,"_b",sep=""))) %>%
  select(stopFacility,xcoord,ycoord,link_idr, isBlocking)

paged_table(center_orem)
```
